...
if __name__ == "__main__":
    ...
    try:
        ...
        create_schema = f"CREATE SCHEMA IF NOT EXISTS {dbname};"
        execute_query(conn, create_schema)
        create_table = f"""CREATE TABLE IF NOT EXISTS {dbname}.test_table (
        ...
        for item in data:
            ...
            values = tuple(item)
            insert_query = f"""INSERT INTO {dbname}.test_table (name, surname, state, age)
                              VALUES (%s, %s, %s, %s);
                           """
            execute_query(conn, insert_query, values)
        cursor = conn.cursor()
       
        count_query = f"SELECT COUNT(*) FROM {dbname}.test_table;" 
        # Выполняем запрос
        ...
        select_where = f'''SELECT MIN(age), MAX(age) 
